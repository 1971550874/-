 ...
 * 功能三：
 */
public class wf_exe_04 {

    static int number = 0;//记录总单词的个数

    public static void main(String[] args) throws IOException {
        if ("-d-s".equals(args[0]) && "-n".equals(args[2])){
            if (args[3] != null)
                readMkdir(args[1], Integer.parseInt(args[3]));
            else readMkdir(args[1],-1);
        }else{
    ...
    public static void readFile_word(String s, HashMap<String,Integer> hashMap) throws IOException {
        ...
        number += word.length;//记录总单词出现的总个数
    ...
    //-d-s
    public static void readMkdir(String s,int n) throws IOException {
        ...
        while (sum-- > 0) {
            ...
            double ans=max*1.0/ number;
            DecimalFormat df = new DecimalFormat("0.00%");
            String r = df.format(ans);
            if (!"".equals(maxword)) {
